        ...
        if account.exchange:
            for exchange_good in account.exchange:
                if exchange_good[0] == good.goodID:
                    ...
                    scheduler.remove_job(job_id=str(account.phone)+'_'+good.goodID)
...
async def _(event: PrivateMessageEvent, matcher: Matcher, state: T_State, bot: Bot, uid = ArgPlainText()):
    ...
    if exchange_plan.result == -1:
        ...
        scheduler.add_job(id=str(account.phone)+'_'+good.goodID, replace_existing=True, trigger='date', func=exchange, args=(exchange_plan, qq), next_run_time=datetime.datetime.strptime(good.time, "%Y-%m-%d %H:%M:%S"))
    ...
    for qq in all_accounts.keys():
        ...
        for account in accounts:
            ...
            for exchange_good in exchange_list:
                ...
                scheduler.add_job(id=str(account.phone)+'_'+exchange_good[0], replace_existing=True, trigger='date', func=exchange, args=(exchange_plan, qq), next_run_time=datetime.datetime.strptime(good_detail.time, "%Y-%m-%d %H:%M:%S"))