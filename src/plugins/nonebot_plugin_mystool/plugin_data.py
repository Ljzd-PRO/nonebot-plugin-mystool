...
from .data_model import BaseModelWithSetter
...
class SaltConfig(BaseSettings):
    ...
    生成Headers - DS所用salt值
    """
    SALT_IOS: str = "0PUWkNIBnLcg8GgRNRJc14kSn4SrPBsS"
    '''生成Headers iOS DS所需的salt 5'''
    SALT_ANDROID: str = "yuzHvf4MkGYyoS4837hHOwLMyVOmtPuY"
    '''生成Headers Android DS所需的salt 2'''
...
class DeviceConfig(BaseSettings):
    ...
    @validator("X_RPC_APP_VERSION", "X_RPC_PLATFORM", allow_reuse=True, pre=True)
    def _(cls, v: str, field) -> str:
        def read_json_file() -> dict:
            ...
            """
            # 找到文件的绝对路径
            with open(PLUGIN_DATA_PATH, "rb") as f:
                data:dict = orjson.loads(f.read())
                return data.get("device_config", {})
        return read_json_file().get(field.name, v)
